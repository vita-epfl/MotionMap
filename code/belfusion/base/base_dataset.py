"""
This file has been modified in the __getitem__ function.
We remove the augmentation, and handle it in other dataloaders.
"""
...
class BaseMultiAgentDataset(Dataset):
    ...
    #new
    def __len__(self):
        # new: E.g.: 12 segments with stride 10 means 2 samples per epoch
        return (len(self.segments) // self.stride) + bool(len(self.segments) % self.stride)

    #new
    def __getitem__(self, sample_idx):
                
        (i, init, end) = self.segments[sample_idx]
        ...
        return obs, pred, {
            ...
            "init": init,
            "end": end,
            "metadata": self.segment_idx_to_metadata[sample_idx]
        }
